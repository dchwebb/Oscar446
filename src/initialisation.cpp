...
void InitLCDHardware(void) {
	...
	// Init RESET pin PC1
	GPIOC->MODER |= GPIO_MODER_MODER1_0;			// 00: Input (reset state)	01: General purpose output mode	10: Alternate function mode	11: Analog mode
	GPIOC->PUPDR |= GPIO_PUPDR_PUPDR1_0;			// Pull up - 00: No pull-up, pull-down; 01 Pull-up; 10 Pull-down; 11 Reserved

	GPIOC->BSRR |= GPIO_BSRR_BR_1;
	GPIOC->BSRR |= GPIO_BSRR_BS_1;

	// Setup SPI pins -  PB0: SPI_MOSI [alternate function AF7]; PB3 SPI_SCK [alternate function AF6]
	GPIOB->MODER |= GPIO_MODER_MODER0_1;			// 00: Input (reset state)	01: General purpose output mode	10: Alternate function mode	11: Analog mode
	GPIOB->OSPEEDR |= GPIO_OSPEEDER_OSPEEDR0;		// V High  - 00: Low speed; 01: Medium speed; 10: High speed; 11: Very high speed
	GPIOB->AFR[0] |= 0b0111 << 0;					// 0b0101 = Alternate Function 5 (SPI5); 4 is position of Pin 9
	...
	GPIOB->AFR[0] |= 0b0110 << 12;					// 0b0101 = Alternate Function 5 (SPI5); 28 is position of Pin 7
...
void InitEncoders() {
	// DISCO: L Encoder: button on PE4, up/down on PE10 and PE11; R Encoder: Button on PA7, up/down on PE8 and PE9
	// L Encoder: button on PC2, up/down on PC10 and PC12; R Encoder: Button on PA7, up/down on PA8 and PA9
	...
	GPIOA->PUPDR |= GPIO_PUPDR_PUPDR8_0;			// Set pin to pull up:  01 Pull-up; 10 Pull-down; 11 Reserved
	SYSCFG->EXTICR[2] |= SYSCFG_EXTICR3_EXTI8_PA;	// Select Pin PA8 which uses External interrupt 3
	...
	GPIOA->PUPDR |= GPIO_PUPDR_PUPDR9_0;			// Set pin to pull up:  01 Pull-up; 10 Pull-down; 11 Reserved
	SYSCFG->EXTICR[2] |= SYSCFG_EXTICR3_EXTI9_PA;	// Select Pin PA9 which uses External interrupt 3
