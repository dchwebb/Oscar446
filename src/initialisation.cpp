...
#ifdef STM32F722xx
	PWR->CR1 |= PWR_CR1_VOS;					// Enable VOS voltage scaling - allows maximum clock speed
#else
...
//	Enable overdrive mode
	...
	while((PWR->CSR1 & PWR_CSR1_ODRDY) != PWR_CSR1_ODRDY);		// Wait till Overdrive is ready
#endif
...
void InitLCDHardware(void) {
	...
	// Init D/CX - choose data (high) or command (low): pin PC0
	GPIOC->MODER |= GPIO_MODER_MODER0_0;			// 00: Input (reset state)	01: General purpose output mode	10: Alternate function mode	11: Analog mode
	GPIOC->OSPEEDR |= GPIO_OSPEEDER_OSPEEDR0_0;		// Medium  - 00: Low speed; 01: Medium speed; 10: High speed; 11: Very high speed

	// Init RESET pin PC1
	GPIOB->MODER |= GPIO_MODER_MODER0_0;			// 00: Input (reset state)	01: General purpose output mode	10: Alternate function mode	11: Analog mode

	//  SPI3 -  PB5: SPI_MOSI [alternate function AF6]
	...
	GPIOB->AFR[0] |= 0b0110 << 20;					// 0b0101 = Alternate Function 6 (SPI3); 20 is position of Pin 5
	...
	GPIOB->AFR[0] |= 0b0110 << 12;					// 0b0101 = Alternate Function 6 (SPI35); 12 is position of Pin 3
	...
	SPI3->CR1 |= SPI_CR1_BR_0;						// Baud rate control prescaler: 0b001: fPCLK/4; 010: fPCLK/8; 0b100: fPCLK/32
	...
	// Initialise TX stream - Stream 5 = SPI3_TX; Channel 0; Manual p207 [221 for F722]
...
void InitADC(void) {
	...
	// Enable ADC - PC3: ADC123_IN13; PA5: ADC12_IN5; PA0: ADC123_IN0
...
#ifdef STM32F722xx
	ADC1->CR2 |= ADC_CR2_EXTSEL_0 | ADC_CR2_EXTSEL_1 | ADC_CR2_EXTSEL_3;	// ADC External trigger: 1011: Timer 2 TRGO
#else
...
void InitEncoders() {
	// L Encoder: button on PC2, up/down on PC10 and PC12; R Encoder: Button on PA7, up/down on PB8 and PB9
	...
	EXTI->FTSR |= EXTI_FTSR_TR2;					// Enable falling edge trigger
	EXTI->IMR |= EXTI_IMR_MR2;						// Activate interrupt using mask register
	...
	// L Encoder using timer functionality - PC6, PC7
	...
	GPIOC->AFR[0] |= 3 << 24;						// Alternate function 3 is TIM8_CH1 (Timer channels in Aleternate function documentation)
	...
	TIM8->SMCR |= TIM_SMCR_ETF;						// Enable digital filter 1111: fSAMPLING=fDTS/32, N=8
	...
	GPIOD->PUPDR |= GPIO_PUPDR_PUPDR12_0;			// Set pin to pull up:  01 Pull-up; 10 Pull-down; 11 Reserved
	GPIOD->MODER |= GPIO_MODER_MODER12_1;			// Set alternate function
	GPIOD->AFR[1] |= 2 << 16;						// Alternate function 2 is TIM4_CH1

	GPIOD->PUPDR |= GPIO_PUPDR_PUPDR13_0;			// Set pin to pull up:  01 Pull-up; 10 Pull-down; 11 Reserved
	GPIOD->MODER |= GPIO_MODER_MODER13_1;			// Set alternate function
	GPIOD->AFR[1] |= 2 << 20;						// Alternate function 2 is TIM4_CH2
